...
- [Cài đặt & Chạy](#cài-đặt--chạy)
  - [Docker Compose (macOS & Linux)](#docker-compose-macos--linux)
  - [Backend](#backend)
  - [Frontend](#frontend)
...
├── Dockerfile                  # Multi-stage: build FE + image API
├── docker-compose.yml          # PostgreSQL + dịch vụ web (BE + FE đã build)
├── compose.env.example         # Mẫu biến môi trường cho Compose (sao chép → compose.env)
├── scripts/                    # Tiện ích CLI + docker-entrypoint + setup.sh
├── tests/                      # Test suite
│   ├── api/                    # API-level tests
...
Chạy **toàn bộ stack bằng Docker** (không cần Python/Node trên máy host) — xem [Docker Compose](#docker-compose-macos--linux).

---

## Cài đặt & Chạy

### Docker Compose (macOS & Linux)

Một lệnh dựng **PostgreSQL**, **FastAPI** và **frontend đã build** (static trong `frontend/dist`, phục vụ cùng cổng 8000 — giống mục [Deploy production](#deploy-production)).

**Yêu cầu**

| Nền tảng | Cài đặt gợi ý |
|----------|----------------|
| **macOS** | [Docker Desktop](https://docs.docker.com/desktop/setup/install/mac-install/) — bật app và đợi daemon sẵn sàng. |
| **Linux** (Debian/Ubuntu) | `sudo apt update && sudo apt install -y docker.io docker-compose-plugin` — sau đó `sudo systemctl enable --now docker` và thêm user vào nhóm `docker` nếu cần: `sudo usermod -aG docker "$USER"` (đăng xuất/đăng nhập lại). |

Cần **Docker Engine** và **Compose** (`docker compose`; một số máy còn có `docker-compose` — script `setup.sh` hỗ trợ cả hai).
...
Script sẽ: kiểm tra Docker đang chạy, tạo `compose.env` từ `compose.env.example` nếu chưa có, rồi `docker compose up -d --build`.

- **Ứng dụng (UI + API cùng origin):** http://localhost:8000  
- **Swagger:** http://localhost:8000/docs  
- **PostgreSQL (host):** `localhost:5432` — user / password / database: `postgres` / `postgres` / `pisces_db` (chỉ dùng cho dev).

**Lần đầu — tạo superadmin**

```bash
docker compose exec web python scripts/create_superadmin.py \
  --email admin@shop.com \
  --full_name "Admin" \
  --password "Admin1234"
```

**Thủ công (tương đương setup.sh)**

```bash
cp compose.env.example compose.env   # chỉnh SECRET_KEY trong compose.env
...
```bash
docker compose logs -f web    # log API
docker compose down           # tắt container (giữ volume DB)
docker compose down -v        # tắt và xóa volume (mất dữ liệu DB trong Docker)
```

Biến môi trường cho container web: file **`compose.env`** (không commit; mẫu là `compose.env.example`). `DATABASE_URL` được gán trong `docker-compose.yml` (host `db`), không cần sửa trừ khi bạn đổi stack.
...
Cách phục vụ SPA + API trên một cổng này trùng với image **`Dockerfile`** dùng trong [Docker Compose](#docker-compose-macos--linux) (build `frontend/dist` rồi chạy uvicorn; production nên tăng `--workers`, đặt `SECRET_KEY` mạnh trong `compose.env` hoặc biến môi trường orchestrator).
