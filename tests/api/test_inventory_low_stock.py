...
async def test_default_unit_price_on_item_and_stock_entry_fallback(client, manager_token):
    cr = await client.post(
        "/api/v1/inventory/items",
        json={
            "name": "OilDefaultPrice",
            "unit": "L",
    ...
    assert cr.status_code == 201
    item = cr.json()
    assert item["default_unit_price"] == 85000
    item_id = item["id"]
    ...
    entry = er.json()
    assert entry["unit_price"] == 85000
    assert entry["total_cost"] == 170000
