...
async def test_list_items_include_ingredients_available(client, waiter_token, db_session):
    cat = await MenuService(db_session).create_category(CategoryCreate(name=f"IA-{uuid.uuid4().hex[:6]}"))
    await MenuService(db_session).create_item(
        MenuItemCreate(name="IA Dish", price=Decimal("5.00"), category_id=cat.id)
    )
    r = await client.get(
        "/api/v1/menu/items",
        headers={"Authorization": f"Bearer {waiter_token}"},
    ...
    data = r.json()
    assert len(data) >= 1
    row = next((x for x in data if x["name"] == "IA Dish"), data[0])
    assert "ingredients_available" in row
    assert isinstance(row["ingredients_available"], bool)
    assert "max_orderable_qty" in row
    assert row["max_orderable_qty"] is None
