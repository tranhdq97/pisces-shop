...
async def test_summary_revenue_reflects_order_discount(db_session):
    ...
    item = await MenuService(db_session).create_item(
        MenuItemCreate(name="Cheap", price=Decimal("100.00"), category_id=cat.id)
    )
    order = Order(
        table_id=None,
        status=OrderStatus.COMPLETED,
    ...
    assert summary.total_revenue == Decimal("70.00")
    assert summary.average_order_value == Decimal("70.00")
    assert summary.top_items[0].total_revenue == Decimal("70.00")
