...
async def test_login_pending_approval_raises_403(db_session):
    from tests.conftest import _make_user
    user = await _make_user(db_session, "pending@test.com", UserRole.WAITER, is_approved=False, is_active=False)
    with pytest.raises(AppException) as exc:
        await AuthService(db_session).login("pending@test.com", "testpassword")
