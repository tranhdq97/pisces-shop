...
function TaskModal({ open, onClose, categoryId, task, onSaved }) {
  ...
  const [penaltyAmount, setPenaltyAmount] = useState(() =>
    task?.penalty_amount != null ? Number(task.penalty_amount) : 0,
  )
  ...
  useEffect(() => {
    ...
    if (task) {
      ...
      setPenaltyAmount(Number(task.penalty_amount ?? 0))
      setSteps((task.steps ?? []).map((s) => ({ ...s, _key: Math.random().toString(36).slice(2) })))
    } else {
      setForm({ title: '', description: '', due_time: '' })
      setPenaltyAmount(0)
      setSteps([])
    }
  }, [open, task])
  ...
  return (
    <Modal open={open} onClose={onClose} title={isEdit ? form.title : t('sop.new_task')}>
      <form onSubmit={submit} className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
        ...
        />
        <MoneyInput
          label={t('sop.penalty_default')}
          value={penaltyAmount}
          onValueChange={setPenaltyAmount}
        />
        <p className="text-xs text-muted -mt-2">{t('sop.penalty_hint')}</p>
        ...
        <div>
          ...
          {selectedRoles.length === 0 && (
            <p className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-lg px-2 py-1.5 mt-2">
              {t('sop.roles_empty_checklist_hint')}
            </p>
          )}
        </div>
      ...
      <div className="flex items-center gap-2 px-4 py-3 bg-slate-50 border-b border-border">
        ...
        <div className="flex items-center gap-1 flex-wrap">
          {cat.allowed_roles?.length > 0
            ...
            : <span className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-full px-2 py-0.5">{t('sop.no_roles_badge')}</span>
          }
        </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {Number(task.penalty_amount) > 0 && (
                    <span className="text-xs bg-amber-50 text-amber-900 border border-amber-200 rounded-full px-1.5 py-0.5">
...
// ── Editor panel (embedded under SOP hub tabs) ───────────────────────────────
export function SOPEditorPanel() {
  const { t } = useT()
