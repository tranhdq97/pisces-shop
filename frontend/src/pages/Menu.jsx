...
function ItemForm({ categories, initial = {}, onSubmit, loading }) {
  ...
  const [form, setForm] = useState({
    ...
    price: initial.price != null && initial.price !== '' ? Number(initial.price) : '',
  ...
  return (
    ...
    >
      ...
      <MoneyInput label={t('menu.price')} value={form.price} onValueChange={(n) => setForm((f) => ({ ...f, price: n }))} required />
...
export default function Menu() {
  ...
  const createItemMutation = useMutation({
    mutationFn: (data) => createItem({ ...data, price: String(Number(data.price)) }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['items'] }); setAddItemOpen(false) },
    onError: (e) => setMutErr(apiErr(e, t)),
  })

  const updateItemMutation = useMutation({
    mutationFn: ({ id, data }) => updateItem(id, { ...data, price: String(Number(data.price)) }),
