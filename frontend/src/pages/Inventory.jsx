...
import {
  ...
  getAllEntries, getItemEntries, addItemEntry, getLowStockItems, deleteEntry,
} from '../api/inventory'
...
function LogTable({ entries, canEdit, onDelete }) {
  const { t } = useT()
  return (
    <table className="w-full text-sm min-w-[560px]">
      <thead className="border-b border-border bg-slate-50 text-xs font-semibold text-muted uppercase tracking-wide">
        <tr>
          ...
          {canEdit && <th className="px-4 py-2 text-right">{t('common.actions')}</th>}
        </tr>
      </thead>
            ...
            {canEdit && (
              <td className="px-4 py-2.5 text-right">
                <button
                  type="button"
                  onClick={() => onDelete?.(e)}
                  className="inline-flex items-center justify-center p-1.5 rounded-lg text-muted hover:bg-red-50 hover:text-red-500"
...
function EntryTable({ entries, unit, canEdit, onDelete }) {
  const { t } = useT()
  return (
...
export default function Inventory() {
  ...
  const deleteEntryMut = useMutation({
    ...
    onError: (e) => { setMutErr(apiErr(e, t)); setConfirmDelEntry(null) },
  })
    ...
    <Layout title={t('inv.title')}>
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                onClick={() => exportCsv('inventory.csv', items, [
                  ...
                  { key: 'default_unit_price', label: t('inv.col_default_price') },
              ...
              <table className="w-full text-sm min-w-[760px]">
                <thead className="border-b border-border bg-slate-50 text-xs font-semibold text-muted uppercase tracking-wide">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right hidden md:table-cell">{t('inv.col_default_price')}</th>
                ...
                <tbody className="divide-y divide-border">
                  {items.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right text-muted hidden md:table-cell tabular-nums">
                        {item.default_unit_price != null
                          ? Number(item.default_unit_price).toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })
                          : '—'}
                      </td>
                            ...
                            <>
                              <button
                                onClick={() => {
                                  ...
                                  setEntryForm({
                                    quantity: '',
                                    unit_price: item.default_unit_price != null ? String(item.default_unit_price) : '',
                                    note: '',
                                  })
                                  setEntryItem(item)
                                }}
            ...
            <div className="space-y-5">
              {Object.keys(logByDay).sort((a, b) => b.localeCompare(a)).map((day) => (
                <div key={day}>
                  ...
                  <div className="bg-card rounded-xl border border-border overflow-x-auto">
                    <LogTable
                      entries={logByDay[day]}
                      canEdit={canEdit}
                      onDelete={(e) => { setMutErr(''); setConfirmDelEntry(e) }}
                    />
                  </div>
                </div>
              ))}
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title={t('inv.new_item_modal')}>
        ...
        <form onSubmit={(e) => { e.preventDefault(); createMut.mutate(itemForm) }} className="space-y-4">
          ...
          />
          <MoneyInput
            label={t('inv.default_unit_price')}
            value={itemForm.default_unit_price}
            onValueChange={(n) => setItemForm((f) => ({ ...f, default_unit_price: n }))}
            placeholder="0"
          />
          {suppliers.length > 0 && (
        ...
        {entryItem && (
          ...
          >
            ...
            <MoneyInput
              ...
              onValueChange={(n) => setEntryForm((f) => ({ ...f, unit_price: n }))}
              placeholder="0"
            />
            <Input
            ...
            : isMultiDayHist ? (
              <div className="space-y-4 -mx-4">
                {Object.keys(entriesByDay).sort((a, b) => b.localeCompare(a)).map((day) => (
                  <div key={day}>
                    <p className="text-xs font-semibold text-muted uppercase tracking-wide mb-1 px-5">{fmtDayHeader(day)}</p>
      ...
      <Modal open={!!confirmDelEntry} onClose={() => setConfirmDelEntry(null)} title="">
        <p className="text-sm text-slate-700 mb-6">
          {t('inv.delete_entry_confirm')}
        </p>
        <div className="flex justify-end gap-3">
          <Button variant="ghost" onClick={() => setConfirmDelEntry(null)} disabled={deleteEntryMut.isPending}>
            {t('common.cancel')}
          </Button>
          <Button
            variant="danger"
            onClick={() => deleteEntryMut.mutate(confirmDelEntry.id)}
            disabled={deleteEntryMut.isPending}
          >
            <Trash2 size={14} />
            {deleteEntryMut.isPending ? t('menu.deleting') : t('common.delete')}
          </Button>
        </div>
      </Modal>
    </Layout>
