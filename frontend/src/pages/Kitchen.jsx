...
import { Clock, ChefHat, RefreshCw, CheckCircle2 } from 'lucide-react'
...
function OrderTicket({ order, status, onStart, onDone, isPending, t }) {
  ...
  // Find the priority item: lowest prep_minutes among unserved items, then 'easy' complexity as tiebreaker
  const unserved = order.details.filter((d) => (d.served_qty ?? 0) < d.qty)
  let priorityItemId = null
  if (status === 'in_progress' && unserved.length > 0) {
    const withTime = unserved.filter((d) => d.prep_minutes != null)
    const candidate = withTime.length > 0
      ? withTime.reduce((a, b) => a.prep_minutes <= b.prep_minutes ? a : b)
      : unserved.find((d) => d.prep_complexity === 'easy') ?? unserved[0]
    priorityItemId = candidate?.item_id
  }

  const allServed = order.details.every((d) => (d.served_qty ?? 0) >= d.qty)
    ...
    <div className={`rounded-xl border-2 p-4 space-y-3 ${CARD_STYLES[status]}`}>
      ...
      <ul className="space-y-1.5">
        {order.details.map((d) => {
          ...
          const done = servedQty >= d.qty
          const isPriority = d.item_id === priorityItemId
          return (
            <li key={d.item_id} className={`flex items-center gap-2 rounded-lg px-2 py-1.5 ${isPriority ? 'bg-white/70 ring-1 ring-amber-300' : ''}`}>
              {done
                ? <CheckCircle2 size={15} className="text-emerald-500 flex-shrink-0" />
                : <span className="w-4 h-4 rounded-full border-2 border-slate-300 flex-shrink-0" />
              }
              <span className={`flex-1 text-sm ${done ? 'line-through text-muted' : 'font-medium text-slate-800'}`}>
                {d.qty}× {d.name}
                {d.prep_minutes && <span className="ml-1.5 text-xs text-muted font-normal">{d.prep_minutes}p</span>}
              </span>
              {isPriority && <span className="text-xs font-bold text-amber-600">{t('kitchen.priority_badge')}</span>}
              {!done && servedQty > 0 && (
                <span className="text-xs text-muted">{t('kitchen.items_progress', { served: servedQty, total: d.qty })}</span>
              )}
            </li>
          )
        })}
        ...
        <div className="flex gap-2 items-center">
          ...
          {status === 'in_progress' && !allServed && (
            <span className="text-xs text-muted">
              {t('kitchen.items_progress', {
                served: order.details.reduce((s, d) => s + (d.served_qty ?? 0), 0),
                total: order.details.reduce((s, d) => s + d.qty, 0),
              })}
            </span>
          )}
        </div>
