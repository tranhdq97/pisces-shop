...
async def lifespan(app: FastAPI):
    ...
    async with AsyncSessionLocal() as session:
        async with session.begin():
            ...
            from app.modules.orders.shop_settings_service import ShopSettingsService  # noqa: PLC0415

            await ShopSettingsService(session).ensure_singleton()
    yield
...
def create_app() -> FastAPI:
    ...
    if FRONTEND_DIST.exists():
        ...
        async def spa_fallback(full_path: str):
            # Serve real files (app-logo.png, icons.svg, etc.) that live in dist root
