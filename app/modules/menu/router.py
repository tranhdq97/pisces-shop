...
async def list_items(
    ...
    max_map = await max_orderable_qty_by_menu_item(db, [i.id for i in items])
    out: list[MenuItemRead] = []
    for i in items:
        base = MenuItemRead.model_validate(i).model_dump()
        mx = max_map.get(i.id)
        base["max_orderable_qty"] = mx
        base["ingredients_available"] = mx is None or mx >= 1
        out.append(MenuItemRead(**base))
    return out
