...
async def list_all_entries(
    ...
    entries = await InventoryService(db).list_all_entries(
        date_from=date_from,
        date_to=date_to,
        direction=direction,
    )
...
async def list_entries(
    ...
    entries = await InventoryService(db).list_entries(
        item_id,
        date_from=date_from,
        date_to=date_to,
        direction=direction,
...
@router.delete("/entries/{entry_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[_inv_edit])
async def delete_entry(entry_id: uuid.UUID, db: AsyncSession = Depends(get_db)) -> None:
    await InventoryService(db).delete_entry(entry_id)
