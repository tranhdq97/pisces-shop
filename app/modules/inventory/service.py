...
class InventoryService:
    ...
    async def add_entry(
        ...
        if unit_price is None and item.default_unit_price is not None:
            unit_price = float(item.default_unit_price)
        entry = StockEntry(
            stock_item_id=item_id,
            quantity=payload.quantity,
            unit_price=unit_price,
            total_cost=(
                round(payload.quantity * unit_price, 2) if unit_price is not None else None
            ),
            note=payload.note,
    ...
    async def get_entry(self, entry_id: uuid.UUID) -> StockEntry:
        result = await self._db.execute(
            select(StockEntry)
            .options(selectinload(StockEntry.stock_item))
            .where(StockEntry.id == entry_id)
        )
        entry = result.scalar_one_or_none()
        if entry is None:
            raise AppException(status_code=404, detail="Stock entry not found.", code="stock_entry_not_found")
        return entry

    async def delete_entry(self, entry_id: uuid.UUID) -> None:
        entry = await self.get_entry(entry_id)
        item = entry.stock_item
        new_qty = float(item.current_quantity) - float(entry.quantity)
        if new_qty < 0:
            raise AppException(
                status_code=409,
                detail="Deletion would result in negative stock quantity.",
                code="stock_quantity_negative",
            )
